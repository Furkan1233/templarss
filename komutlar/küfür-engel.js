...
exports.run = async (client ,message, args) =>{
    if(!message.member.roles.cache.has('ayarlamakistediğinrolünidsi')) return message.channel.send(new Discord.MessageEmbed().setColor('BLACK').setFooter(client.user.username, client.user.avatarURL()).setDescription(`uyarımesajı`)).then(msg => {msg.delete({ timeout: 15000 })});
if(args[0] === 'aç') {
